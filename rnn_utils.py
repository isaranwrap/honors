...
sys.path.append('/home/ishan/honors_thesis/github/')
...
def create_dataset(jaFolder, num_samples=int(1e5),
    ...
    #create empty matrices to populate, later cut up into Xtrain, Ytrain, s.t.
    #dim0 --> size 32 (batch size) filled with 32 different joint time series (2 overlap)
    #dim1 --> the number of time points needed to create enough batches with to match the num_samples specified parameter, it's num_batches * lookback... it's about 3 times more than num_samples (since lookback/batch_size ~ 3.1) 
    #dim2 --> number of angles, 30 
    Xuncut = np.empty((batch_size, ja_slice, num_angles))
    Yuncut = np.empty((batch_size, ja_slice, num_angles))
    
    #fill each row with a time series of length 
    for indx, file in enumerate(os.listdir(jaFolder)):
        ja = h5py.File(os.path.join(jaFolder,file))['joint_angles']
        rnd = np.random.randint(ja.shape[0]-ja_slice)
        Xuncut[indx,...] = ja[rnd:rnd+ja_slice]
        Yuncut[indx,...] = ja[rnd+depth:rnd+ja_slice+depth]
...
def draw_line(arr, ax, point1, point2, dtype = 'pos', **kwargs):
        xs = [arr[0, point1], arr[0, point2]]
        ys = [arr[1, point1], arr[1, point2]]
        ax.plot(xs, ys, **kwargs)
    
def draw_skeleton(arr=None, means=None, color = 'red', linewidth=0.7, input_type='ang', **kwargs):
    if input_type == 'pos':
        fig = plt.figure(**kwargs)
        ax = fig.add_subplot(111)
        for p in range(6, 9, 1):
            draw_line(arr, ax,p, p+1, color=color, linewidth=linewidth)
        ...
        for p in range(26, 29, 1):
            draw_line(arr, ax,p, p+1, color=color, linewidth=linewidth)
        draw_line(arr, ax,2, 0, linewidth=linewidth)
        draw_line(arr, ax,1, 0, linewidth=linewidth)
        draw_line(arr, ax,5, 4, linewidth=linewidth)
        draw_line(arr, ax,4, 3, color='purple', linewidth=linewidth)
        draw_line(arr, ax,30, 3, linewidth=linewidth)
        draw_line(arr, ax,31, 3, linewidth=linewidth)
        for i in range(32):
            ax.scatter(arr[0,i], arr[1,i], marker='$' + str(i+1)+ '$', s=200)
    ...
    if input_type == 'ang':
       ...
       [ 94.9058512 ,  78.1094025 , 111.35902611,  94.62192528,
        ...
        for i in range(32):
            ax.scatter(ja_[i], mean_lngth[i], marker='$' + str(i+1)+ '$', s=200)
        ax.set_yticklabels([])
        #ax.set_xticklabels([])
        ax.xaxis.grid(False)
        ax.yaxis.grid(False)
...
def writeData(X_train, Y_train, X_test, Y_test, dtype = 'train_val', outFolder=''):
    if not os.path.exists(outFolder) and outFolder != '':
        os.mkdir(outFolder)
    if dtype == 'train_val':
        with h5py.File(os.path.join(outFolder, 'Xtrain.h5'), 'w') as h5f:
            h5f.create_dataset('joint_angles', data=X_train)
        with h5py.File(os.path.join(outFolder, 'Ytrain.h5'), 'w') as h5f:
            h5f.create_dataset('joint_angles', data=Y_train)
        with h5py.File(os.path.join(outFolder, 'Xval.h5'), 'w') as h5f:
            h5f.create_dataset('joint_angles', data=X_test)
        with h5py.File(os.path.join(outFolder, 'Yval.h5'), 'w') as h5f:
            h5f.create_dataset('joint_angles', data=Y_test)
    elif dtype == 'val_test':
        with h5py.File(os.path.join(outFolder, 'Xval.h5'), 'w') as h5f:
            h5f.create_dataset('joint_angles', data=X_train)
        with h5py.File(os.path.join(outFolder, 'Yval.h5'), 'w') as h5f:
            h5f.create_dataset('joint_angles', data=Y_train)
        with h5py.File(os.path.join(outFolder, 'Xtest.h5'), 'w') as h5f:
            h5f.create_dataset('joint_angles', data=X_test)
        with h5py.File(os.path.join(outFolder, 'Ytest.h5'), 'w') as h5f:
            h5f.create_dataset('joint_angles', data=Y_test)
    return 'done!'

def createJATimeSeries(dataFolder='/home/ishan/princeton_data/preds/', outFolder='/home/ishan/honors_thesis/joint_angles/', median_filter = False):
    for file in os.listdir(dataFolder):
        pos = h5py.File(dataFolder + file)['positions_pred']
        ja = getJointAngles(pos[1000:])
        if median_filter:
            for ang in range(num_angles):
                ja[:,ang] = medfilt(ja[:,ang], kernel_size=3)
        if not os.path.exists(outFolder):
            os.mkdir(outFolder)
        with h5py.File(outFolder + 'ja_{}'.format(file), 'w') as h5f:
            h5f.create_dataset('joint_angles', data = ja)
        print('{} out of {} done'.format(len(os.listdir(outFolder)), len(os.listdir(dataFolder))))
    return 'done!'
